...
export default {
  data() {
    return {
      ...
      detecteTime: null,
      connectStatus: false,
      wsBaseReqid: Math.floor(Math.random() * 100000) * 10000,
    };
  },
  created() {
    if (this.webSocketConfigs) {
  ...
  methods: {
    ...
    sendSocketData(url, cmd, data) {
      const wi = this.webSocketInstances[url];
      if (wi && wi.ins && wi.ins.readyState === WebSocket.OPEN) {
        this.wsBaseReqid = this.wsBaseReqid + 1;
        const req = {
          cmd,
          data,
          reqid: this.wsBaseReqid,
          timsstamp: new Date() / 1000,
        };
        console.log(`websocket send:${url},cmd:${cmd},data:${JSON.stringify(data)}`);
        wi.ins.send(JSON.stringify(req));
      }
    },
