<template>
  <el-modal
    class="tedge-active-warning-detail"
      ...
      >
        ...
        >
          <template slot="label">
            ...
            >
              {{ dtl.ruleInfo.restoreExpression }}
            </el-help-tip>
            告警源
          </template>
        ...
        >
          <el-tag
            :type="data.eventStatus === 1 ? 'danger' : 'success'"
            effect="plain"
          >
            {{ statusMap[data.eventStatus] }}
          </el-tag>
        </el-form-item>
        <el-form-item
          label="处理建议"
...
import { warning as cgi, tbosWarning as tbosCgi } from 'module/tedge/config/cgi';
...
export default {
  ...
  methods: {
    ...
    JumpOrigin(data) {
      // console.log(deviceNumber, 'deviceNumberdeviceNumber');
    ...
    fetchHistory() {
      ...
      getEdgeRequest(this.$axios, this.mozuId).post(cgi.getWarningHistory, parm, false)
        .then((data) => {
    ...
    async fetchInfo() {
      await getEdgeRequest(this.$axios, this.mozuId).post(cgi.getWarningDetail, {
        AlarmId: this.id,
        MozuId: this.mozuId,
      }, false)
        .then(async (data) => {
    ...
    async fetchChartData() {
      ...
      if (this.$moduleInfo?.isTbos) {
        ...
        const list = result?.list.map((i) => {
          ...
          console.log(i?.point_data, '处理前');
          item.pointDataList = (i?.point_data.map(j => ({
            time: j.update_time,
            val: j.value,
          })) || []).filter(item => item.time && dayjs(item.time).diff(dayjs()) <= 0);
          console.log(item.pointDataList, '处理后');
          return {
            ...item,
          };
        });
