...
type evaluationStage struct {
	...
	// primarily used for specific operators that don't care which side a given type is on,
	// but still requires one side to be of a given type
	// (like string concat)
...
func addStage(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {
	left, right, err := transferTokenToVal(left, right, parameters)
	if err != nil {
		return nil, err
	}
	...
	if isBool(left) {
		left = transBoolToFloat64(left.(bool))
	}
	if isBool(right) {
		right = transBoolToFloat64(right.(bool))
	}

	return left.(float64) + right.(float64), nil
}

// transBoolToFloat 转换bool型为浮点数
func transBoolToFloat64(v bool) float64 {
	f := float64(0)
	if v == true {
		f = 1
	}
...
// transFloat64ToBool 转换浮点数为bool型
func transFloat64ToBool(v float64) bool {
	f := false
	if v > 0 {
		f = true
	}
...
func andStage(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {
	...
	if isFloat64(left) {
		left = transFloat64ToBool(left.(float64))
	}
	if isFloat64(right) {
		right = transFloat64ToBool(right.(float64))
	}
...
func regexStage(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {
	left, right, tErr := transferTokenToVal(left, right, parameters)
	if tErr != nil {
		return nil, tErr
	}
...
func typeConvertParam(p reflect.Value, t reflect.Type) (ret reflect.Value, err error) {
	defer func() {
		if r := recover(); r != nil {
			errorMsg := fmt.Sprintf("Argument type conversion failed: failed to convert '%s' to '%s'", p.Kind().String(),
				t.Kind().String())
	...
	if numIn != numParams {
		if numIn > numParams {
			return nil, fmt.Errorf(
				"too few arguments to parameter call: got %d arguments, expected %d", len(params), numIn)
		}
		return nil, fmt.Errorf(
			"too many arguments to parameter call: got %d arguments, expected %d", len(params), numIn)
	}
...
func makeAccessorStage(pair []string) evaluationOperator {
	...
	return func(left interface{}, right interface{}, parameters Parameters) (ret interface{}, err error) {
		...
		for i := 1; i < len(pair); i++ {
			...
			return nil, errors.New("Method call '" + pair[0] + "." + pair[1] +
				"' did not return either one value, or a value and an error. Cannot interpret meaning.")
		}
...
func additionTypeCheck(left interface{}, right interface{}) bool {
	...
	//算法加支持bool型相加 支持1.0的跨设备表达式
	if (isFloat64(left) || isBool(left)) && (isFloat64(right) || isBool(right)) {
		return true
	}
	if !isString(left) && !isString(right) {
		return false
...
func transferTokenToVal(left interface{}, right interface{}, parameters Parameters) (interface{}, interface{}, error) {
	...
	nright := right
	if isString(left) {
		valList, ok := parameters.GetExpression().CustomData[ExprCustomDataKeyData].(epoint.SymValueMapList)[left.(string)]
		if !ok {
			return left, right, nil
		}
		if len(valList) > 1 {
			return left, right, fmt.Errorf(
				"simple expr has more than one gid map, left sym: %s not match", left.(string))
		}
		val, ok := valList[0][0]
		if !ok {
			return left, right, fmt.Errorf("cannot find data for left sym: %s", left.(string))
		}
		nleft = val
	}
	if isString(right) {
		valList, ok := parameters.GetExpression().CustomData[ExprCustomDataKeyData].(epoint.SymValueMapList)[right.(string)]
		if !ok {
			return left, right, nil
		}
		if len(valList) > 1 {
			return left, right, fmt.Errorf(
				"simple expr has more than one gid map, right sym: %s not match", right.(string))
		}
		val, ok := valList[0][0]
		if !ok {
			return left, right, fmt.Errorf("cannot find data for right sym: %s", right.(string))
		}
		nright = val
	}
