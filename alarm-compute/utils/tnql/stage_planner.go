...
Most `precedent` functions can be described by the same function, 
just with different type checks, symbols, and error formats.
...
/*
	...
	The three stages of evaluation can be thought of as parsing strings to tokens, 
	then tokens to a stage list, then evaluation with parameters.
*/
...
// planFunction TODO
/*
	A special case where functions need to be of higher precedence than values, 
	and need a special wrapped execution stage operator.
*/
...
// planValue TODO
/*
	A truly special precedence function, 
	this handles all the "lowest-case" errata of the process, including literals, parmeters,
...
/*
	...
	For commutative operators like "+" or "-", it's no big deal. But for order-specific operators, 
	it ruins the expected result.
*/
...
*/
func elideLiterals(root *evaluationStage) (*evaluationStage, error) {

	if root.leftStage != nil {
		g, err := elideLiterals(root.leftStage)
		if err != nil {
			return nil, err
		}
	...
	if root.rightStage != nil {
		g, err := elideLiterals(root.rightStage)
		if err != nil {
			return nil, err
		}
...
*/
func elideStage(root *evaluationStage) (*evaluationStage, error) {
